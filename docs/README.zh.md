...
**自进化个人 SNS 平台** — 你拥有推荐算法,并用自然语言与行为反馈持续调校方向。
...
大多数信息工具是**手** — 只取回你指向的东西。Hedwig 是**你拥有的 Feed 引擎** — 你用自然语言设定方向,系统从滚动、停留、跳过、upvote/downvote 等行为中持续校准。
...
| **算法所有权** | 企业(YouTube、X)或固定(开源工具) | **你拥有并可导出** |
| **算法方向调校** | 平台隐式决定 | **自然语言 + 显式反馈 + 行为信号** |
| **魔鬼代言人** | 无 | 每个信号都包含反对观点 |
...
3. **Algorithm Steering** — 自然语言指令、Q&A 接受/拒绝、upvote/downvote、dwell/skip/share 汇入同一个 fitness。
...
<p align="center">
  <i>决定哪些信息到达你的算法应该属于你,也应该能由你调校方向。</i>
</p>
