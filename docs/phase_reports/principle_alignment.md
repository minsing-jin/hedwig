...
| 2 | **자가진화 + 자연어 주도권** | "스스로 진화하는 추천알고리즘이고, 사용자가 원하는대로 자연어로 방향성을 지시" |
...
| **2.1** 자연어로 criteria 편집 | `onboarding/nl_editor.py` + `/criteria/propose` | ✅ 충족 | — |
| **2.2** 자연어로 algorithm 편집 | `onboarding/nl_algo_editor.py` + `/algorithm/propose` | ✅ 충족 | — |
...
| **2.6** Quad-Input (NL + Q&A + vote + dwell) | NL editor / `/qa/feedback` / `/feedback/{id}/{vote}` / `behavior_events` beacon | ✅ 충족 | 모두 단일 `evolution_signal` 스트림으로 수렴 |
