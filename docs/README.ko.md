...
**자기진화 개인 SNS 플랫폼** — 내가 소유한 추천 알고리즘을 자연어와 행동신호로 방향 조정합니다.
...
Hedwig는 **내가 소유한 피드 엔진**입니다 — 당신이 자연어로 방향을 잡고, 스크롤·스와이프·👍/👎 같은 행동신호가 그 방향을 계속 보정합니다.

| | YouTube / X | 다른 도구 | **Hedwig** |
|---|---|---|---|
| **알고리즘 소유** | 기업 | 고정 / 오픈소스 | **당신** |
| **알고리즘 방향 조정** | 플랫폼이 암묵적으로 결정 | 설정 파일 직접 편집 | **자연어 + 피드백 + 행동신호** |
...
3. **Algorithm Steering** — 자연어 지시, Q&A 수용, 👍/👎, dwell/skip/share가 하나의 fitness로 수렴
...
4. /feed 에서 스크롤·스와이프하며 시그널 소비
5. 필요하면 /chat 에 "논문 비중 높여", "crypto 줄여"처럼 방향 조정
6. 👍/👎와 dwell/skip/share가 쌓이고 시스템이 진화
```
...
<p align="center">
  <em>당신에게 도달하는 정보를 결정하는 알고리즘은 당신의 것이고, 당신이 방향을 조정할 수 있어야 합니다.</em>
</p>
